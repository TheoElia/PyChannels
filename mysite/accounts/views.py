...
from .forms import CustomUserCreationForm,PatronCreationForm,RetailerChangeForm,RetailerCreationForm,PatronChangeForm
...
class CreateRetailerView(TemplateView):
    var1 = "heloo"
    form_class = RetailerCreationForm
    success_url = reverse_lazy('home')
    template_name = 'create_user.html'

    def get(self, request):
        form = self.form_class
        args={'form':form,'var1':self.var1}
        return render(request,self.template_name,{'form':form})
    ...
    def post(self,request):
        ...
        if form.is_valid():
            ...
            password = form.cleaned_data['password1']
            user = self.form_valid(form)
            if not self.is_member(user.id, 'Retailers'):
                user.groups.add(group)
                user.admin = request.user.id
                user.admin_name = request.user.username
...
class UserListView(ListView):
    model = CustomUser
    template_name = "accounts/para.html"
...
class UserDetailView(DetailView):
    model = CustomUser
    template_name = "accounts/para_detail.html"
    def get_context_data(self, **kwargs):
